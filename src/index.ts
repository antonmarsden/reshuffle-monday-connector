...
> {
  ...
  private columnValueUpdatersToObject(
    ...
    for (const [key, updater] of Object.entries(columnValues)) {
      ...
      newValues[bc.id] = typeof newValue !== 'object' ? String(newValue) : newValue
    }
  ...
  async createItem(
    ...
    const boardColumnsResp = await this.getColumn(boardId)
    ...
    const res = await this.query(CREATE_ITEM_QUERY, {
      ...
      column_values:
        columnValues && boardColumns
          ? JSON.stringify(this.columnValueUpdatersToObject(columnValues, boardColumns))
          : undefined,
    })
